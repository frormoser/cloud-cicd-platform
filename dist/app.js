// Minimal frontend for the GitHub profile demo (dist bundle).
// Same behavior as the source: tries backend first, falls back to GitHub public API.
...
async function fetchProfile(username) {
  ...
  try {
    ...
    // otherwise fall through to public GitHub API
  } catch (err) {
  ...
  return {
    ...
    profile: {
      ...
      top_repos: (repos || []).
        ...
        slice(0, 6).
        map(r => ({ name: r.name, html_url: r.html_url, stars: r.stargazers_count, language: r.language })) ,
      languages,
